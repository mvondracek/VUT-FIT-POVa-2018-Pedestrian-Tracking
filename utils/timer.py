...
from detector import OpenPoseBinaryDetector
from openpose.PovaPose import PovaPose
...
def openpose_cpu_binary(openpose_binary_path=None, repeat=3):
    ...
    detection_time = timeit.repeat(detection,
                                   globals={'person_detector': person_detector, 'image': image},
                                   number=1, repeat=repeat)
    print("{} single image t={}".format(openpose_cpu_binary.__name__, detection_time))
    ...
    detection_time = timeit.repeat(detection,
                                   globals={'person_detector': person_detector, 'images': images, 'cameras': cameras},
                                   number=1, repeat=repeat)
    print("{} {} images t={}".format(openpose_cpu_binary.__name__, image_count, detection_time))


def main():
    povapose_single_image_single_person()
    # openpose_gpu_binary(openpose_binary_path=r'C:\openpose-1.4.0-win64-cpu-binaries\bin\OpenPoseDemo.exe')
    # openpose_cpu_binary(openpose_binary_path=r'C:\openpose-1.4.0-win64-cpu-binaries\bin\OpenPoseDemo.exe')
